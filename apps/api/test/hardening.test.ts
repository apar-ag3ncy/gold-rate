...
describe('config fails fast', () => {
  ...
  it('accepts a complete production env and lists every problem otherwise', () => {
    ...
    // no login screen: auto-login is allowed in production (the deploy script sets it); the old name still works
    expect(() => loadConfig({ ...base, AUTO_LOGIN_EMAIL: 'admin@x.test' })).not.toThrow();
    expect(() => loadConfig({ ...base, DEV_AUTO_LOGIN_EMAIL: 'admin@x.test' })).not.toThrow();
  });
});
